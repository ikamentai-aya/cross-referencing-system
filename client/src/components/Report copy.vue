<template>
  <div>
    <div class="container-fluid">
          ...
          <v-sheet>
            <v-row align="center" justify="space-around">
              ...
              <a href="#" class="btn" @click="changeReport">
                <v-icon color=#69696>mdi-swap-horizontal</v-icon>
              </a>
              <note-taking
              ...
              ></note-taking><a href="#" class="btn" @click="changeDisplayNote" width="50" >
                <v-icon color=#696969>mdi-book-open</v-icon>
              </a>
            </v-row>
          </v-sheet>
        </div>
      ...
      <hr>
      <note-display v-show="doDisplayNote"></note-display>
      <div class="row"  style="margin : 2px 2px 2px 2px"  id="row" v-show="!doDisplayNote">
        ...
        <div class="col-1" v-show="!isParagraph && !isRefer">
          <div id="my_viz2"></div>
        </div>
        <div class="col-1" v-show="!isParagraph && !isRefer" id="cluster_btn">
          <p><small>Group</small></p>
          <div v-for="g in cluster_group" :key="g">
            <a href="#" class="btn"
              @click="selectCluster(g.index)" :style="g.color">
            {{g.name}}</a>
          </div>
        </div>
...
export default {
  ...
  data() {
    return {
      ...
      menu_list: [
        { title: '何となく大事', icon: 'mdi-check', color: '#d9267f' },
  ...
  methods: {
    ...
    clickReport(e) {
      ...
      console.log(el.clientHeight, el.clientWidth, el);
      const payload = {
        x: e.offsetX,
        y: e.offsetY,
    ...
    changeReport() {
      if (this.isReport) {
        this.isReport = false;
        this.selectCluster(this.select_cluster);
      } else {
        this.isReport = true;
        // クラスターのヒートマップをもとに戻す
        this.cluster_svg.selectAll('rect.heatmap')
    ...
    hideRefer() {
      ...
      // メモの送信
        const payload = {
          ...
          note: '',
          id: 0,
          stamp: this.chip_item[this.chip_refer],
